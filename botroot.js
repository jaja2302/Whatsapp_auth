...
async function connectToWhatsApp() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    //             conversation.indexOf("*Nama* : ") + "*Nama* : ".length;
    ...
    //                 await sock.sendMessage(
    //                   noWa,
    //                   { text: "Izin Berhasil di approved" },
    //                   { quoted: message }
    //                 );
    //               }
    //             } catch (error) {
    ...
    //               // const response = await axios.post('http://qc-apps2.test/api/updatenotifijin', {
    //               const response = await axios.post(
    //                 // "http://127.0.0.1:8000/api/rejectedoreder",
        ...
        if (message.message?.extendedTextMessage?.contextInfo) {
          ...
          if (contextInfo.quotedMessage &&contextInfo.quotedMessage.conversation) {
            ...
            if (conversation.includes('Izin baru perlu di approved')) {
              ...
              const idPemohonStartIndex = conversation.indexOf('*ID* : ') + '*ID* : '.length;
              const idPemohonEndIndex = conversation.indexOf('\n',idPemohonStartIndex);
              const idPemohon = conversation.substring(idPemohonStartIndex, idPemohonEndIndex).trim();
              const [id] = idPemohon.split('/').map((part) => part.trim());
              if (respon_atasan.toLowerCase() !== 'ya' &&respon_atasan.toLowerCase() !== 'tidak') {
                ...
                try {
                  ...
                  let responses = response.data;await sock.sendMessage(noWa, {text: `${responses.message}`});
                } catch (error) {
                  console.log('Error approving:', error);
                }
              } else if (respon_atasan.toLowerCase() === 'tidak') {
                ...
                try {
                  // const response = await axios.post('http://qc-apps2.test/api/updatenotifijin', {
                  const response = await axios.post('http://127.0.0.1:8000/api/rejectedoreder',
                    {
                      email: 'j',
                      password: 'j',
                      id: id,
...
app.get('/testing', async (req, res) => {
  try {
    await handleBotLaporanHarianTraksi(sock);
